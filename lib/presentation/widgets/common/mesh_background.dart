...
class _EyeCatchingMeshPainter extends CustomPainter {
  ...
  void _paintLightMode(Canvas canvas, Size size) {
    ...
    canvas.drawCircle(const Offset(100, 100), 380, glow1Paint);

    final glow2Paint = Paint()
      ..shader = ui.Gradient.radial(
        Offset(size.width - 100, 500),
        350,
        [
    ...
    canvas.drawCircle(Offset(size.width - 100, 500), 350, glow2Paint);
    ...
    for (int i = 0; i <= numCols; i++) {
      for (int j = 0; j <= numRows; j++) {
        if ((i + j) % 4 == 0) {
          dots.add(Offset(i * spacing, j * spacing));
        }
      }
    }
